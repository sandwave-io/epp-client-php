...
class ContactQueryTransferResponseTest extends TestCase
{
    public function test_response(): void
    {
        $response = new ContactQueryTransferResponse(Document::fromString((string) file_get_contents(__DIR__ . '/../data/responses/contact_transfer.xml')));
        ...
        $this->assertInstanceOf(Carbon::class, $response->getRequestDate(), 'request date not set on contact query transfer.');
        $this->assertSame('ClientY', $response->getAcceptanceClientId());
        $this->assertInstanceOf(Carbon::class, $response->getAcceptanceDate(), 'acceptance date not set on contact query transfer.');
    }
}
